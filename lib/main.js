(function() {
  ...
  this._compile_jobdef = function(jobdef) {
    ...
    /* TAINT simplify this with next version of InterType:
     return new Error report if ( report = @types.xxxxxxx.mixa_settings jobdef )?
     or similar, as the case may be */
    // validate.mixa_settings jobdef
    if (!isa.mixa_settings(jobdef)) {
      aspect = this.types._get_unsatisfied_aspect('mixa_settings', jobdef);
  ...
  this.run = function(jobdef, argv = null) {
    var R;
    if (this.types.is_sad((R = this.parse(jobdef, argv)))) {
